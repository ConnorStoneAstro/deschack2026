...
importance_weights_denom = 1 / jax.scipy.stats.norm.pdf(
    (mu_scat - mu_true)[select], loc=0, scale=mu_scat_std
)
...
class NormalizedLikelihood(NaiveLikelihood):
    @ck.forward
    def log_P_D_C(self):
        update_mu_true = 5 * jnp.log10(jax.vmap(self.cosmology.luminosity_distance)(z_true)) - 5
        importance_weights_numer = jax.scipy.stats.norm.pdf(
            (mu_scat - update_mu_true)[select],
            loc=0,
            scale=M_std,  # Note using the forward model scatter here
        )
        P_D_C = jnp.sum(importance_weights_numer * importance_weights_denom) / Nsamp
        return jnp.log(P_D_C)
